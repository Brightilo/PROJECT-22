...
* Create an NGINX pod manifest   apiVersion: v1
*   kind: Pod
*   metadata:
*     name: nginx-pod
*   spec:
...
* Access the pod through its IP from within the K8s cluster kubectl run curl --image=dareyregistry/curl -i --tty
*   curl -v 172.50.202.214:80
*   

* Create a service to access to Nginx pod   apiVersion: v1
*   kind: Service
*   metadata:
*     name: nginx-service
*   spec:
...
*       - protocol: TCP
*         port: 80
*         targetPort: 80  kubectl apply -f nginx-service.yaml
*  
* Update the pod manifest to include the labels metadata apiVersion: v1
*   kind: Pod
*   metadata:
*       name: nginx-pod
*       labels:
...
* Use port forwarding to attach port 8089 on the node to port 80 on the service kubectl  port-forward svc/nginx-service 8089:80
*   
